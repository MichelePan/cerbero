...
def render_expo_fx_v6():
 ...
 .slot-container { border: 2px solid #555; border-radius: 8px; padding: 15px; margin-bottom: 25px; background-color: #ffffff; box-shadow: 2px 2px 5px rgba(0,0,0,0.1); }
 ...
 .slot-header { font-size: 18px; font-weight: bold; color: #333; margin-bottom: 10px; border-bottom: 1px solid #ddd; padding-bottom: 5px; }
 ...
 def callback():
 current_val = st.session_state[source_key]
 st.session_state[target_key] = "SELL" if current_val == "BUY" else "BUY"
 return callback

 def make_sync_date_callback(source_key, target_key):
 def callback():
 st.session_state[target_key] = st.session_state[source_key]
 return callback

 def make_sync_pipv_callback(source_key, target_key):
 def callback():
 st.session_state[target_key] = st.session_state[source_key]
 return callback
 ...
 st.session_state[k(f"s{i}_t1_type")] = t1_def
 st.session_state[k(f"s{i}_t1_sym")] = sym1
 st.session_state[k(f"s{i}_t1_open")] = 0.00000
 st.session_state[k(f"s{i}_t1_swap")] = 0.00
 st.session_state[k(f"s{i}_t1_pipv")] = 0.00

 st.session_state[k(f"s{i}_t2_date")] = default_date
 st.session_state[k(f"s{i}_t2_type")] = t2_def
 st.session_state[k(f"s{i}_t2_sym")] = sym2
 st.session_state[k(f"s{i}_t2_open")] = 0.00000
 st.session_state[k(f"s{i}_t2_swap")] = 0.00
 st.session_state[k(f"s{i}_t2_pipv")] = 0.00
 ...
 with c_d:
 st.number_input("DISPY", value=st.session_state.get(k("dispy"), 0.0), format="%.2f", key=k("dispy"))
 with c_c:
 st.number_input("CTRV", value=st.session_state.get(k("ctrv"), 0.0), format="%.2f", key=k("ctrv"))
 st.markdown("---")

 if st.session_state.get(k("do_update"), False) or not st.session_state.get(k("results")):
 ...
 t1_open = st.session_state.get(k(f"s{i}_t1_open"), 0.0)
 t1_swap = st.session_state.get(k(f"s{i}_t1_swap"), 0.0)
 t1_pipv = st.session_state.get(k(f"s{i}_t1_pipv"), 0.0)
 ...
 t2_open = st.session_state.get(k(f"s{i}_t2_open"), 0.0)
 t2_swap = st.session_state.get(k(f"s{i}_t2_swap"), 0.0)
 t2_pipv = st.session_state.get(k(f"s{i}_t2_pipv"), 0.0)

 res = {"t1_act": 0.0, "t1_val": 0.0, "t1_err": None, "t2_act": 0.0, "t2_val": 0.0, "t2_err": None, "net_series": None}
 ...
 res["t1_act"] = df1["Close"].iloc[-1]
 res["t1_val"] = s1.iloc[-1]
 else:
 res["t1_err"] = "Err Calc"
 ...
 res["t2_act"] = df2["Close"].iloc[-1]
 res["t2_val"] = s2.iloc[-1]
 else:
 res["t2_err"] = "Err Calc"
 ...
 all_chart_traces.append({"name": f"Slot {i}", "df": res["net_series"], "color": SLOT_COLORS[i - 1]})

 group_series_s = pd.Series(net_pnl.values, index=net_pnl.index)
 if 1 <= i <= 5:
 group_series["A"].append(group_series_s)
 elif 6 <= i <= 10:
 group_series["B"].append(group_series_s)
 else:
 group_series["C"].append(group_series_s)

 new_results[i] = res

 st.session_state[k("results")] = new_results
 st.session_state[k("chart_data")] = all_chart_traces
 st.session_state[k("dispy_val")] = st.session_state[k("dispy")]
 st.session_state[k("ctrv_val")] = st.session_state[k("ctrv")]
 ...
 current_dispy = st.session_state.get(k("dispy_val"), 0.0)
 current_ctrv = st.session_state.get(k("ctrv_val"), 0.0)

 if current_chart_data:
 all_dfs = [t["df"]["Date"] for t in current_chart_data]
 all_dates = pd.concat(all_dfs)
 min_x = all_dates.min()
 max_x = all_dates.max()
 else:
 ...
 fig.add_shape(type="line", x0=min_date, y0=0, x1=max_date, y1=0, line=dict(color="black", width=2, dash="dash"))

 for i in range(1, 16):
 data = current_results.get(i, {"t1_act": 0.0, "t1_val": 0.0, "t1_err": None, "t2_act": 0.0, "t2_val": 0.0, "t2_err": None})
 ...
 with c1:
 st.date_input("DATE", key=k(f"s{i}_t1_date"), on_change=make_sync_date_callback(k(f"s{i}_t1_date"), k(f"s{i}_t2_date")))
 with c2:
 ...
 with c5:
 if data["t1_err"]:
 st.metric("ACTUAL", data["t1_err"])
 else:
 st.metric("ACTUAL", f"{data['t1_act']:.5f}")
 with c6:
 st.number_input("SWAP/D", value=0.0, format="%.2f", key=k(f"s{i}_t1_swap"))
 with c7:
 st.number_input("PIPV", value=0.0, format="%.2f", key=k(f"s{i}_t1_pipv"), on_change=make_sync_pipv_callback(k(f"s{i}_t1_pipv"), k(f"s{i}_t2_pipv")))
 with c8:
 ...
 with d1:
 st.date_input("DATE", key=k(f"s{i}_t2_date"), on_change=make_sync_date_callback(k(f"s{i}_t2_date"), k(f"s{i}_t1_date")))
 with d2:
 ...
 with d5:
 if data["t2_err"]:
 st.metric("ACTUAL", data["t2_err"])
 else:
 st.metric("ACTUAL", f"{data['t2_act']:.5f}")
 with d6:
 st.number_input("SWAP/D", value=0.0, format="%.2f", key=k(f"s{i}_t2_swap"))
 with d7:
 st.number_input("PIPV", value=0.0, format="%.2f", key=k(f"s{i}_t2_pipv"), on_change=make_sync_pipv_callback(k(f"s{i}_t2_pipv"), k(f"s{i}_t1_pipv")))
 with d8:
 display_colored_value(data["t2_val"], data["t2_err"])

 st.markdown("### Grafico Riferimento (DISPY / CTRV)")
 fig_ref = go.Figure()
 fig_ref.add_trace(go.Scatter(x=[min_x, max_x], y=[current_dispy, current_dispy], mode="lines", name="DISPY", line=dict(color="black", dash="dash", width=2), hoverinfo="name+y"))
 fig_ref.add_trace(go.Scatter(x=[min_x, max_x], y=[current_dispy, current_ctrv], mode="lines", name="CTRV", line=dict(color="black", width=3), hoverinfo="name+y"))
 ...
 x=trace["df"]["Date"], y=trace["df"]["Value"], mode="lines",
 name=trace["name"], line=dict(color=trace["color"], width=3),
 hovertemplate="<b>%{fullData.name}</b><br>Data: %{x|%Y-%m-%d}<br>Valore: %{y:.2f}<extra></extra>"
 ))
 ...
 fig_slots.update_layout(height=500, hovermode="x unified", legend=dict(orientation="h", yanchor="bottom", y=1.02, xanchor="right", x=1))
 ...
 master_dates = np.sort(all_dates_list)
 ...
 x=total_df_all["Date"], y=total_df_all["Total"], mode="lines",
 name="Totale (1-15)", line=dict(color="#FF0000", width=4),
 hovertemplate="<b>Totale 1-15</b><br>Data: %{x|%Y-%m-%d}<br>Valore: %{y:.2f}<extra></extra>"
 ))

 if current_group_totals.get("A") is not None:
 fig_total.add_trace(go.Scatter(
 x=current_group_totals["A"]["Date"], y=current_group_totals["A"]["Value"], mode="lines",
 name="Totale (1-5)", line=dict(color="#FFD800", width=3),
 hovertemplate="<b>Totale 1-5</b><br>Data: %{x|%Y-%m-%d}<br>Valore: %{y:.2f}<extra></extra>"
 ))
 if current_group_totals.get("B") is not None:
 fig_total.add_trace(go.Scatter(
 x=current_group_totals["B"]["Date"], y=current_group_totals["B"]["Value"], mode="lines",
 name="Totale (6-10)", line=dict(color="#0000FF", width=3),
 hovertemplate="<b>Totale 6-10</b><br>Data: %{x|%Y-%m-%d}<br>Valore: %{y:.2f}<extra></extra>"
 ))
 if current_group_totals.get("C") is not None:
 fig_total.add_trace(go.Scatter(
 x=current_group_totals["C"]["Date"], y=current_group_totals["C"]["Value"], mode="lines",
 name="Totale (11-15)", line=dict(color="#00FF00", width=3),
 hovertemplate="<b>Totale 11-15</b><br>Data: %{x|%Y-%m-%d}<br>Valore: %{y:.2f}<extra></extra>"
 ))
 ...
 hovertemplate=f"<b>{name}</b><br>Data: %{{x|%Y-%m-%d}}<br>Valore: %{{y:.2f}}<extra></extra>",
 ))
