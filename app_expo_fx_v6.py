...
def render_expo_fx_v6():
    ...
    def k(name: str) -> str:
        """Prefix helper for widget/session_state keys."""
    ...
    st.markdown(
        """
        <style>
        /* Colora di giallo gli input manuali */
        ...
        /* BORDO GIALLO (Slot 1-5) */
        .slot-yellow {
          border: 3px solid #FFD700 !important;
          background-color: #fffff0;
        }

        /* BORDO BLU COBALTO (Slot 6-10) */
        .slot-blue {
          border: 3px solid #0047AB !important;
          background-color: #f0f8ff;
        }

        /* BORDO ROSSO (Slot 11-15) */
        .slot-red {
    ...
    # --- 2. FUNZIONI DI CALCOLO E DOWNLOAD ---
    def download_data(symbol, start_d, end_d):
        """Scarica dati reali SENZA controlli sul futuro."""
        if not symbol:
            return None
        ...
        try:
            ...
            print(f"Errore download {sym}: {e}")
            return None

    def calculate_daily_pnl(df, open_p, type_t, pipv, swap_d):
        """Calcola il PnL giornaliero."""
        ...
        for price in closes:
            days_counter += 1
            if type_t == "SELL":
                raw_diff = open_p - price
            else:
                raw_diff = price - open_p
    ...
    # --- 3. GESTIONE STATO ---
    def reset_all():
        # cancella solo chiavi dell’app, non tutto lo stato globale
    ...
    # Init stato una sola volta (namespaced)
    ...
    for i in range(1, 16):
        ...
        with c1:
            st.date_input(
                "DATE",
                key=k(f"s{i}_t1_date"),
                on_change=make_sync_date_callback(k(f"s{i}_t1_date"), k(f"s{i}_t2_date")),
            )
        with c2:
            st.selectbox(
                "TYPE",
                ["BUY", "SELL"],
                key=k(f"s{i}_t1_type"),
                on_change=make_opposite_callback(k(f"s{i}_t1_type"), k(f"s{i}_t2_type")),
            )
        with c3:
        ...
        with c7:
            st.number_input(
                ...
                value=0.0,
                format="%.2f",
                key=k(f"s{i}_t1_pipv"),
                on_change=make_sync_pipv_callback(k(f"s{i}_t1_pipv"), k(f"s{i}_t2_pipv")),
            )
        with c8:
        ...
        with d1:
            st.date_input(
                "DATE",
                key=k(f"s{i}_t2_date"),
                on_change=make_sync_date_callback(k(f"s{i}_t2_date"), k(f"s{i}_t1_date")),
            )
        with d2:
            st.selectbox(
                "TYPE",
                ["BUY", "SELL"],
                key=k(f"s{i}_t2_type"),
                on_change=make_opposite_callback(k(f"s{i}_t2_type"), k(f"s{i}_t1_type")),
            )
        with d3:
        ...
        with d7:
            st.number_input(
                ...
                value=0.0,
                format="%.2f",
                key=k(f"s{i}_t2_pipv"),
                on_change=make_sync_pipv_callback(k(f"s{i}_t2_pipv"), k(f"s{i}_t1_pipv")),
            )
        with d8:
            display_colored_value(data["t2_val"], data["t2_err"])

    # --- 7. GRAFICO 1: RIFERIMENTO (DISPY / CTRV) ---
    ...
    if current_chart_data:
        ...
        for trace in current_chart_data:
            ...
            show_trace = False
            if g2_filter == "Visualizza solo slot utilizzati":
                show_trace = True
            elif "1 a 5" in g2_filter and 1 <= slot_num <= 5:
                show_trace = True
            elif "6 a 10" in g2_filter and 6 <= slot_num <= 10:
                show_trace = True
            elif "11 a 15" in g2_filter and 11 <= slot_num <= 15:
                show_trace = True
    ...
    # --- 9. GRAFICO 3: TOTALE ESPOSIZIONE ---
    ...
    if current_chart_data:
        ...
        if current_group_totals.get("A") is not None:
            fig_total.add_trace(
                go.Scatter(
                    x=current_group_totals["A"]["Date"],
                    y=current_group_totals["A"]["Value"],
                    mode="lines",
                    name="Totale (1-5)",
                    line=dict(color="#FFD800", width=3),
        ...
        if current_group_totals.get("B") is not None:
            fig_total.add_trace(
                go.Scatter(
                    x=current_group_totals["B"]["Date"],
                    y=current_group_totals["B"]["Value"],
                    mode="lines",
                    name="Totale (6-10)",
        ...
        if current_group_totals.get("C") is not None:
            fig_total.add_trace(
                go.Scatter(
                    x=current_group_totals["C"]["Date"],
                    y=current_group_totals["C"]["Value"],
                    mode="lines",
                    name="Totale (11-15)",
    ...
    if "Tutti e 3" in g4_view:
        ...
        if current_group_totals.get("B") is not None and current_group_totals.get("C") is not None:
            ...
            sum_val = df_b.reindex(idx_union).ffill().fillna(0).add(df_c.reindex(idx_union).ffill().fillna(
