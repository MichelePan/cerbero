...
def render_fore_forecast_fx():
    ...
    with st.container(border=True):
        ...
        with c1:
            selected_pair_1 = st.selectbox(
                "Tasso di Cambio 1",
                options=list(ticker_map.keys()),
                key="ff_pair1",
            )
            selected_pair_2 = st.selectbox(
                "Tasso di Cambio 2",
                options=list(ticker_map.keys()),
                key="ff_pair2",
            )
        ...
        with c2:
            ...
            freq_choice = st.selectbox(
                "Frequenza Dati Storici",
                options=list(freq_options_map.keys()),
                key="ff_freq",
            )
    ...
    with st.spinner("Scaricamento dati (cache 30 min), elaborazione e calcolo modello..."):
        try:
            ...
            rmse1 = (
                float(np.sqrt(mean_squared_error(df1.loc[common_idx1, "Valore"], preds1.loc[common_idx1])))
                if len(common_idx1) > 0
                else 0.0
            )

            common_idx2 = df2.index.intersection(preds2.index)
            rmse2 = (
                float(np.sqrt(mean_squared_error(df2.loc[common_idx2, "Valore"], preds2.loc[common_idx2])))
