...
def download_fx_close_cached(tickers: tuple, start_date_iso: str):
...
def render_weighing_fx():
    ...
    start_date_iso = start_date.replace(microsecond=0).isoformat()

    with st.spinner("Scarico dati (cache 30 min) e calcolo statistiche..."):
        try:
            data = download_fx_close_cached(tuple(selected_fx), start_date_iso)
